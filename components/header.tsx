...
export function Header({ onSettingsClick, onHomeClick, onAnnouncementsClick, onReportsClick, onInviteClick, activeTab, onTabChange }: HeaderProps) {
  ...
  const businessItems = [
    { label: "100's List", href: "/prospect-tracker", description: "Track your pipeline" },
    { label: "Client List", href: "/client-tracker", description: "Touchpoints & milestones" },
    { label: "Coach List", href: "/coach-tracker", description: "Track your team's progress" },
  ...
  return (
    <header className="border-b border-optavia-border bg-white sticky top-0 z-50">
      <div className="container mx-auto px-4">
        ...
        {/* Desktop Navigation Menu - Order: My Business | Calendar | Coaching Resource Library | Coaching Quicklinks | Meal Planning & Recipes | Outside Tools & Resources | Metabolic Reset Events */}
        {user && (
          <nav className="hidden md:flex items-center justify-center flex-wrap gap-x-2 lg:gap-x-4 xl:gap-x-6 gap-y-1 border-t border-optavia-border py-2">
            {/* My Business Dropdown - Positioned first, hidden for training-only orgs */}
            {!isTrainingOnly && (
              <DropdownMenu>
        ...
        {user && mobileMenuOpen && (
          <nav className="md:hidden border-t border-optavia-border bg-white">
            <div className="flex flex-col">
              {/* My Business Section first - Hidden for training-only orgs */}
              {!isTrainingOnly && (
                <>
