...
interface MeetingEmailRecipient {
  to: string
  fullName: string
}

interface BatchMeetingEmailRequest {
...
/**
 * Generate meeting email HTML content
 */
function generateMeetingEmailContent(
  fullName: string,
  meetingTitle: string,
  ...
  const callTypeColor = callType === "coach_only" ? "#7c3aed" : "#14b8a6"
  ...
  const html = getEmailWrapper(header + bodyContent + footer, "New Meeting Scheduled")
...
/**
 * POST /api/send-meeting-email-batch
 * Send meeting notification emails to multiple recipients using Resend batch API
 */
export async function POST(request: NextRequest) {
  try {
    const body: BatchMeetingEmailRequest = await request.json()
    ...
    if (!meetingTitle || !meetingDate || !meetingTime) {
      return NextResponse.json(
        { error: "Missing required fields: meetingTitle, meetingDate, meetingTime" },
        { status: 400 }
      )
    }

    // Limit batch size (Resend limit is 100)
    ...
    const batchEmails = recipients.map(recipient => {
      const { subject, html, text } = generateMeetingEmailContent(
        recipient.fullName,
        meetingTitle,
        meetingDescription,
        meetingDate,
    ...
    const successCount = data?.data?.filter(r => r?.id)?.length || 0
    ...
    console.error("Batch meeting email error:", error)
    return NextResponse.json(
